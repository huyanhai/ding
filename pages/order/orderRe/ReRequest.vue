	...
	<view>
		...
		<view class="goods-section">
			...
			<view class="flexend padding1 mt10">
				<view @click="Rerequest(1)" class="mr20">
					共：<text style="color: #EBA542;">￥{{total}}元</text>
				</view>
			</view>
		</view>
		...
		<view class="padding1 bgcolor10 mt10">
			<view>
				<view class="font10 fontB padding10" v-if='type==0'>
					退款原因
				</view>
				...
				<view class="padding10" style="padding-top: 0;">
					<picker @change="bindPickerChange" :value="index" :range="reason" style="color: #EBA542;">
					    <view>{{index==undefined?'请选择原因':reason[index]}}</view>
					</picker>
				</view> -->
			</view>
		</view>
		...
		<view class="padding1 bgcolor10 mt10">
			<view>
				...
				<view class="padding10" style="padding-top: 0;">
					<textarea placeholder="请将您的售后原因写下来，以便解决售后问题" style="width: 96%;height: 50rpx;font-size: 11px;" maxlength="400" v-model="desc" />
				</view>
			</view>
		</view>
		...
		<view class="bgcolor10 padding1 mt10">
			<view class="flexaround" style="padding: 20rpx 40rpx;">
				...
				<view class="font10 fontB box_col_75">
					<input type="number" placeholder="请输入您的联系号码" v-model="phone" maxlength="11"/>
				</view>
			</view>
		</view>
...
<script>
	export default {
		...
		async onLoad(option) {
			...
			if(option.type==1){
				uni.setNavigationBarTitle({title: '退款'})
			}
			this.getOrderInfo()
			this.getReason()
			this.getQiniu()
		...
		methods: {
			async submit(){
				...
				let data = {
					...
					productId :this.goodsData[0].productId,
					quantity: this.goodsData[0].productQuantity,
					reason: this.reason[this.index]
				}
				let result = await this.$http.refundExchange(data)
				if(result.code==200){
					this.$api.msg(result.message)
					uni.redirectTo({
						url:'reResult?returnId='+result.data
					})
				}else{
					this.$api.msg(result.message)
				}
			...
			async getOrderInfo() {
				...
				for(let item of that.goodsData){
					that.total += item.productPrice * item.productQuantity
				}
			},

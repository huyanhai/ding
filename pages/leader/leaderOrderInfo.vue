	...
	<view>
		<view v-if="orderInfo" class="orderStatus">
			<view class="g-header" v-if="orderInfo.storeName">
				<image class="logo" :src="orderInfo.storeIcon"></image>
			...
			<view class="padding1">
				...
				<view class="flexbetween">
					...
					<!-- <view class="addressInfo mt10 font8" v-if='orderInfo.isGift==1 && orderInfo.orderStatus == 1' style="border:1px solid #272A30;border-radius: 10rpx;padding: 2px;">{{orderInfo.receiverName?'地址已获得，等待发货':'好友未填写地址，24小时后将自动退单'}}</text></view> -->
				</view>
			</view>
		</view>
		...
		<view class="goods-section">
			...
			<view class="g-item" v-for="(item, index) in goodsData" :key="item.productId">
				...
				<view class="right">
					...
					<view class="flexend">
						<!-- <view class="action-btn afterSales" @click="afterSales(item.productId)" v-if='orderInfo.orderStatus == 2 && item.status===undefined'>
							申请售后
						</view> -->
						<view class="action-btn mt10" style="height: 60rpx;line-height: 60rpx;color: #888;border: 1px solid #888;background-color: white;" v-if='orderInfo.orderStatus == 2 && item.status!==undefined'>
							...
							<text v-if='item.status==4'>已撤销</text>
						</view>
					</view>
				</view>
			</view>
		...
		<view class="footer">
			<view class="price-content-right flexcenter">
				<view v-if="buttonItem.type!='share'" class="action-btn" @click.stop="orderBtn(orderInfo,buttonItem.type)" v-for="buttonItem in orderInfo.buttons">
					{{buttonItem.name}}
				</view>
			</view>
		</view>
...
<script>
	export default {
		...
		methods: {
			...
			async reOrder(){
				// let orderItem;
				// let result = await this.$http.getOrderInfo({orderId:this.orderInfo.orderId});
				// if (result.code == 200) {
				...
				// }
				// uni.navigateTo({
				// 	url: `/pages/order/createOrder?type=2&orderId=${this.orderInfo.orderId}&isGiving=${this.orderInfo.isGift}&data=${JSON.stringify({
				// 		goodsData: goodInfo,
				// 	})}`
				// })
				uni.showLoading({
			...
			// 	let result = await this.$http.cancelOrder({
			...
			async getOrderInfo() {
				...
				let result = await this.$http.getStoreOrderDetail(option);
				uni.hideLoading();
				if (result.code == 200) {
					if (result.data) {
						that.orderInfo = result.data;
						that.goodsData = that.orderInfo.orderItem;
						// if (that.orderInfo.orderStatus == 1 && !that.orderInfo.receiveNickname) {
						// 	let time = Number(((new Date(that.orderInfo.createTime)).getTime() + 86400000))
						// 	let now = parseInt((new Date()).getTime() - (new Date()).getTime()%1000)
						// 	console.log(time, now)
						// 	that.leftTime = time - now 
						// 	if (that.leftTime >= 0) {
						// 		let timer = setInterval(function() {
						// 			that.leftTime -= 1000
						// 			that.timer = that.setTimes(that.leftTime)
						// 			if (that.leftTime == 0) {
						// 				clearInterval(timer)
						// 				this.getOrderInfo();
						// 			}
			...
			async pay() {
				...
				if (this.orderInfo.payAmount == 0) {
					...
					// uni.navigateTo({
					// url:`/pages/money/makePay?orderId=${this.orderInfo.orderId}`
					// })

<template>
	<view class="container">
		<live-pusher id='livePusher' ref="livePusher" class="livePusher" :url="liveInfo.pushUrl"
		        mode="SD" :muted="true" :enable-mic="isMic" :enable-camera="isStart" :auto-focus="true" :beauty="beauty" whiteness="2"
			...
			<cover-view style="height: 88vh;display: flex;flex-direction: column;justify-content: flex-end;">
				...
				<cover-view class="bottom">
					...
					<!-- <input class="input" :focus="isFocus" type="text" v-model="msg" @blur="onBlusr" @confirm='confirm' confirm-type="send"/> -->
					<cover-view style="margin-right: 35upx;display: flex;">
						<cover-image @click="switchCamera" src="../../static/tabCamera1.png" class="bottom-icon"></cover-image>
						<cover-image @click="togglePushStatus" :src="isPushing?'../../static/pause.png':'../../static/play.png'" class="bottom-icon"></cover-image>
						<cover-image @click="toggleBeauty" src="../../static/meiyan.png" class="bottom-icon"></cover-image>
						<cover-image @click="toggleMic" :src="isMic?'../../static/voice.png':'../../static/closeVoice.png'" class="bottom-icon"></cover-image>
					</cover-view>
				</cover-view>
			</cover-view>
			<cover-view class="startPush" v-if="!isStart" @click="startPush">
				开始直播
			</cover-view>
			<!-- 商品面板 -->
			<cover-view class="coupon-mask" :class="maskState===0 ? 'none' : maskState===1 ? 'show' : ''" @click="toggleMask" @touchmove.stop.prevent="stopPrevent">
				<cover-view class="coupon-mask-content" @click.stop.prevent="stopPrevent">
					<cover-view style="display: flex;justify-content: space-between;margin-top: 30upx;width: 94%;margin-left: 3%;">
						...
						<!-- <cover-view class="addProduct" @click="setGroupAdminer">添加助手</cover-view> -->
						<cover-view class="addProduct" v-if="!isStart" @click="goAddProduct">添加商品</cover-view>
					</cover-view>
					<cover-view class="coupon-mask-content" @click.stop.prevent="stopPrevent" v-if='plist.length>0' style="width: 94%;margin-left: 3%;overflow-y: scroll;max-height: 60vh;">
						<cover-view class="p-list">
							<cover-view  class="p-item" v-for="(item,index) in plist" :key="item.id">
								...
								<cover-view class="p-button">
									<cover-view class="p-btn" @click="removeProduct(item,index)">移除</cover-view>
								</cover-view>
							</cover-view>
						</cover-view>
					</cover-view>
					<cover-view v-if='plist.length==0' class="textcenter mtrpx100">
						<cover-view class="font10 fontcolor16">你还没有添加商品哦~</cover-view>
					</cover-view>
				</cover-view>
			</cover-view>
		</live-pusher>
...
<script>
    ...
    export default {
		...
		async onShow(){
			...
			// 		if(this.plist[i].skuId == tempProduct.skuId){
			// 			this.$api.msg('该商品已添加');
			// 			return;
			// 		}
			// 	}
        ...
        async onLoad(option) {
			...
			// let gift = await this.$http.giftCategoryList();
			// let gift1 = await this.$http.giftList({category_id:gift.data[1].id});
			// console.log(gift1)
			this.login();
			this.param = {
				mermber_phone:this.memberInfo.phone,
				live_sn:option.live_sn,
			}
			let res = await this.$http.pushLive(this.param)
			if(res.code==200){
				console.log(res)
        ...
        methods: {
			// setGroupAdminer(){
			// 	let options = {
			// 		groupID:this.liveInfo.room_id,
			// 		userID:'xxy',
			// 		role:TIM.TYPES.GRP_MBR_ROLE_ADMIN
			// 	}
			// 	let promise = this.tim.setGroupMemberRole(options);
			...
			async login(){
				...
				that.tim.on(TIM.EVENT.SDK_READY, function(event) {
					...
					console.log(that.liveInfo.room_id)
					...
					promise.then(function(imResponse) {
					  ...
					  switch (imResponse.data.status) {
					    ...
					    case TIM.TYPES.JOIN_STATUS_SUCCESS: // 加群成功
						  ...
						  // that.sendMessage('我进入了直播间','enter')
					      break
			...
			async togglePushStatus() {
				if(this.isPushing){
					this.context.pause({
						success: async (a) => {
							console.log("livePusher.pause:" + JSON.stringify(a));
							this.isPushing = false;
							// let res = await this.$http.endLive(this.param)
							// if(res.code==200){
							// 	console.log(res)
							// }else{
							// 	this.$api.msg(res.msg)
			...
			startPush(){
				...
				this.context.start({
					success: async (a) => {
						...
						this.isStart = true;
						uni.hideLoading()
						let res = await this.$http.startLive(this.param)
						if(res.code==200){
							console.log(res)
						}else{
							// this.$api.msg(res.msg)

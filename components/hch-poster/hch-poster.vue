<template>
  ...
  <view class="canvas_box" :hidden="canvasFlag">
    ...
    <image src="../../static/close_btn.png" type="cancel" class="canvas_close_btn" size="60" @tap="canvasCancelEvn" /><!-- 关闭 -->
    <view class="button-wrapper">
      <!-- 保存海报按钮 -->
      <button class="save_btn" @tap="saveCanvasImage">保存图片</button>
    </view>
  </view>
</template>
...
export default {
  ...
  methods: {
    ...
    createCanvasImage(tab,_pthis) {
	  ...
	  let url = '../../static/hbg.png';
      // let zpPriceIcon = this.posterObj.icon //图标
      let code = this.posterObj.code
      let closeBtn = this.posterObj.closeUrl
	  let imageH = 400;
	  let top = 100;
	  let imageW = 225*scaleW;
	  this.y = top * scaleH;
	  this.x = (_this.phoneW - imageW) / 2;
	  this.width = imageW
	  this.height = imageH * scaleH;
      ...
      wx.getImageInfo({
        ...
        success(res) {
          ...
          // _this.roundRect(
          //   ctx,
          //   (_this.phoneW - (_this.phoneW - 130)) / 2,
          //   top * scaleH,
          //   _this.phoneW - 130,
          //   imageH * scaleH,
          //   10,
      ...
      // ctx.drawImage("../../static/close_btn.png", 50 + (_this.phoneW - 100) + 5, 70, 24, 24)
      // ctx.draw(true)
      // 关闭按钮 end
   //    if(tab!='index'){
		 //  // 海报商品title
		 ...
		 //        row[b],
		 //        leftSpace,
		 //        (contentTextY + textLineHeight * b - 15) * scaleH,
		 //        this.phoneW - 130
		 //      )
		 //      ctx.draw(true)
		 //    }
   ...
   //    if(tab!='index'){
		 ...
		 //    orignPriceW
		 //  ) //5价格间距
		 //  ctx.moveTo(80 + zpPriceW + 5, 392 * scaleH) //设置线条的起始路径坐标
		 //  ctx.lineTo(80 + zpPriceW + 5 + orignPriceW, 392 * scaleH) //设置线条的终点路径坐标
   ...
   //    ctx.font = "normal bold 14px sans-serif"
   //    ctx.fillText("丁老表", (_this.phoneW - 55) / 2, 530 * scaleH, 90)
   //    // 小程序的名称end
   //    // 长按/扫描识别查看商品
   //    ctx.setFontSize(14)
   ...
   //    ctx.font = "normal bold 14px sans-serif"
   //    ctx.fillText("保存图片", (_this.phoneW - 58) / 2,( saveBtnTop+23) * scaleH, 58)
      //绘制保存按钮 end
      wx.hideLoading()
    },
    ...
    saveCanvasImage() {
      ...
      uni.canvasToTempFilePath({
        x: this.x,
        y: this.y,
        width: this.width, // 画布的宽
        height: this.height, // 画布的高
        destWidth: this.phoneW*4,
        destHeight: this.phoneH*4,
        canvasId: "myCanvas",
        success(res) {
          // 2-保存图片至相册

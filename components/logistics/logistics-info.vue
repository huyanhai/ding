...
<!-- 						<text class="ali_top_icon" style="margin: 0 0;" v-if="item.stateName=='签收'">收</text>
						<text class="ali_top_icon" style="margin: 0 0;" v-else-if="item.stateName=='发货'">发</text>
						<text class="ali_top_icon" style="margin: 0 0;" v-else-if="item.stateName=='待收货'">待</text> -->
						<!-- =====如果不够，可以再方法和过滤器中同时添加判断-----或者如下的--[v-else-if]操作 -->
						<!-- <text class="ali_top_icon" style="margin: 0 0;" v-else-if="item.content.includes('丰巢智能快递柜') && i!= datas.length-1">待</text> -->
						<!-- <text class="ali_top_icon" :class="[item.content.includes('签收')?'isOk':'']" style="margin: 0 0;" v-else-if="Type(item.remark) == true && i!= datas.length-1">{{item.remark | Type}}</text> -->
						<view class="ali_top_icons" style="margin: 0 0;" >
							<text class="line" :class="[i == 0?'noline':'']"></text>
							<text class="ali_top_iconss"></text>
							<text class="line" :class="[i == 0?'noline':'']"></text>
						</view>

	...
	export default {
		...
		methods: {
			async getAllArea() {
				...
				//默认获取省份列表
				this.getadd(1,this.areaList[0].id);
				//默认获取城市列表
				this.getadd(2,this.areaList[0].children[0].id);
				//默认获取地区列表
				//this.getadd(3, this.ct);
				// 声明默认地址,并传送给父组件
				this.emitData();
			...
			getadd(flag, id) {
				switch (flag) {
					case 1:
						_self.addressd1 = this.areaList;
						break;
					case 2:
						var newArr = [];
						for (var i = 0; i < this.areaList.length; i++) {
							if (this.areaList[i].id == id) {
								newArr = this.areaList[i].children;
							}
						}
					...
					case 3:
						...
						var newArr = [];
						for (var i = 0; i < this.areaList.length; i++) {
							if(this.areaList[i].children) {
								var tmpArr = this.areaList[i].children;
								for(var j = 0; j < tmpArr.length; j++) {
									if (tmpArr[j].id == id) {
										newArr = tmpArr[j].children;
									}
								}
							}
						}
						_self.addressd3 = newArr;
						_self.ctname = newArr[0].name;
						_self.ct = newArr[0].id;
						break;
					default:

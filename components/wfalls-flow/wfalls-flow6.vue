<template>
	<view class="list-container" :style="{'background-color': background}">
		...
		<view class="item" v-for="(item,index) of viewList" :key="index" v-if="item.status!=2">
			...
			<view style="width: 100%;display: flex;justify-content: space-between;margin-top: 32upx;" class="padding1">
				<view @click="editLive(item)" style="width: 146upx;height: 48upx;border-radius: 48upx;color: #1DC7AC;font-size: 24upx;border: 1upx solid #1DC7AC;text-align: center;line-height: 48upx;">
					编辑内容
				</view>
				<view @click="startLive(item)" style="width: 146upx;height: 48upx;border-radius: 48upx;color: #fff;font-size: 24upx;background-color: #1DC7AC;text-align: center;line-height: 48upx;">
					开始直播
				</view>
			</view>
		</view>
...
<script>
	export default {
		...
		methods: {
			startLive(item){
				uni.navigateTo({
					url:"/pages/live/livePush?live_sn="+item.live_sn+"&room_id="+item.room_id
				})
			},
			editLive(item){
				uni.navigateTo({
					url:"/pages/live/applyLive?type=1&live_sn="+item.live_sn
				})
			},
            async init(){
